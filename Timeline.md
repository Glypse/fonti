...
0. [x] User preferences: prefer .otf, .ttf static or .ttf variable
    - Auto-detect variable fonts through `fontTools`
1. [x] Install GitHub fonts through the `install user/repo version` syntax
2. [x] Remove fonts installed **via fontpm only** through the `uninstall user/repo` syntax\*
3. [x] Updates fonts installed **via fontpm only** through the `update` syntax\*
    - Changelog preview

2 and 3 mean that we have to keep a list of all installed fonts; figure out the best architecture for that. Ideas:

-   [x] Hash the installed file, keep track of its path (and name) as well as hash for identification of the exact file
...
1. [ ] Support for more than GitHub; ideally any Git remote with tagged commits as "releases"
    - [ ] Support for GitLab
2. [x] Support for non-global installations (example use case: woff2 install insisde of a web app repo)
